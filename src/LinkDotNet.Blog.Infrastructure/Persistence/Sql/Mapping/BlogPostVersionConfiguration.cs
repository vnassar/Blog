...
internal sealed class BlogPostVersionConfiguration : IEntityTypeConfiguration<BlogPostVersion>
{
    public void Configure(EntityTypeBuilder<BlogPostVersion> builder)
    {
        ...
        builder.Property(c => c.Id).IsUnicode(false).ValueGeneratedOnAdd();

        builder.Property(x => x.BlogPostId).HasMaxLength(256).IsRequired().IsUnicode(false);
        ...
        builder.HasIndex(x => new { x.BlogPostId, x.VersionNumber })
            .IsUnique()
            .HasDatabaseName("IX_BlogPostVersions_BlogPostId_VersionNumber");
